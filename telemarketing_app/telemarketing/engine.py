...
from .llm_service import TelemarketingLLMService
...
class TelemarketingEngine:
    def __init__(
        ...
        self.llm_service = TelemarketingLLMService(self.settings)
    ...
    def chat(
        ...
        *,
        ...
        preview_hits = self._retrieve_hits(cleaned)
        llm_analysis = self.llm_service.analyze_turn(
            user_text=cleaned,
            state=state,
            extracted=extracted,
            fallback=analysis,
        ...
        llm_reply = self.llm_service.rewrite_reply(
            user_text=cleaned,
            state=state,
            analysis=analysis,
            draft_reply=result["reply"],
            hits=result.get("retrieved_hits", []),
        )
        draft_reply = llm_reply or result["reply"]
        reply = self._finalize_reply(draft_reply, state, analysis)
        result["reply"] = reply
    ...
    def _analyze_turn(self, user_text: str, state: ConversationState) -> dict:
        ...
        topic = self._detect_topic(user_text) or state.last_topic
        ...
        elif state.risk_flag == "high" and any(token in user_text for token in ("承诺", "保证", "直接给我", "说死", "包过")):
            intent = "risky_request"
            reasons.append("高风险话题连续追问")
        elif state.conversation_outcome == "do_not_contact" and any(
            token in user_text for token in ("不需要", "别联系", "不要打", "拒绝过", "别再")
        ):
            intent = "stop_request"
            reasons.append("用户已标记不再联系，继续保持停止联系")
        ...
        elif any(token in user_text for token in STOP_CALL_KEYWORDS):
            intent = "stop_request"
            reasons.append("用户明确要求停止联系")
        elif any(token in user_text for token in PRIVACY_KEYWORDS):
            intent = "privacy_concern"
            reasons.append("用户在质疑号码来源或隐私")
        elif any(token in user_text for token in ROBOT_KEYWORDS):
            intent = "identity_check"
            reasons.append("用户在确认是否真人或机器人")
        elif any(token in user_text for token in IDENTITY_KEYWORDS) or (
            state.current_intent == "identity_check" and any(token in user_text for token in ("机构", "来意", "做什么", "找我"))
        ):
            intent = "identity_check"
        ...
        elif extract_grade(user_text) or extract_subjects(user_text) or "孩子" in user_text:
            intent = "sales_lead"
            reasons.append("命中年级/学科/孩子信息，继续销售流程")
    ...
    def _build_response(self, user_text: str, state: ConversationState, analysis: dict) -> dict:
        ...
        if analysis["intent"] == "stop_request":
            return self._handle_stop_request(state)
        if analysis["intent"] == "privacy_concern":
            return self._handle_privacy(state)
    ...
    def _handle_stop_request(self, state: ConversationState) -> dict:
        state.conversation_outcome = "do_not_contact"
        reply = "抱歉打扰您了。我这边先为您标记为不再主动联系，后续不会继续电话跟进。如果您之后主动想了解课程、试听或售后，再直接找我们就行。"
        return {
            "reply": reply,
            "intent": "stop_request",
            "reply_type": "answer",
            "next_action": "respect_stop_request",
            "retrieved_hits": [],
            "strategy": "用户明确拒绝时停止销售推进并确认不再联系",
        }

    def _handle_privacy(self, state: ConversationState) -> dict:
        state.conversation_outcome = "privacy_reassured"
        reply = (
            "抱歉让您觉得被打扰了。关于号码来源，我这边不能替第三方渠道做未经核实的说明。"
            "如果您不希望继续联系，我现在就可以为您标记停止联系；如果您只是想先确认机构身份和来意，我也可以直接说清楚。"
        )
        return {
            "reply": reply,
            "intent": "privacy_concern",
            "reply_type": "answer",
            "next_action": "privacy_reassure_or_stop",
            "retrieved_hits": [],
            "strategy": "先安抚隐私顾虑，再给出停止联系或继续确认的选项",
        }
    ...
    def _handle_objection(self, user_text: str, state: ConversationState, analysis: dict) -> dict:
        ...
        if objection_type == "busy" and "关键" not in reply and "重点" not in reply:
            reply = f"我就说关键一点。 {reply}"
        if next_step:
    ...
    def _handle_faq(self, user_text: str, state: ConversationState, analysis: dict) -> dict:
        ...
        if not hits and topic:
            topic_query = self._topic_query(topic)
            if topic_query:
                hits = self._retrieve_hits(topic_query, preferred_type="qa")
    ...
    def _topic_query(self, topic: str) -> str:
        mapping = {
            "location": "校区 地址 在哪 线上",
            "price": "价格 报价 费用",
            "trial": "试听 试课 安排",
            "teacher": "老师 师资",
            "online": "线上 网课",
        ...
        *,
        ...
        workflow = [
            ...
            self._workflow_step(
                "LLM层",
                "调用大模型做意图判断/回复润色",
                "已启用" if self.llm_service.enabled else f"未启用或已回退({self.llm_service.disabled_reason or 'llm_disabled'})",
            ),
            self._workflow_step(
                "话术层",
